...
import { LayoutDashboard, Users, Target, Terminal, Settings, LogOut, Cpu, Radio, Menu, X, ShieldCheck, User, MessageCircle, Book, Search, WifiOff, Lock } from 'lucide-react';
...
export const Layout: React.FC<LayoutProps> = ({ children, currentView, onNavigate, onReset, userProfile }) => {
  ...
  const navItems = [
    ...
    { id: 'SCRAPER', label: 'نظام الزحف (Scraper)', icon: Search },
    { id: 'INBOX', label: 'الرد المباشر', icon: MessageCircle },
    { id: 'LOGS', label: 'نواة النظام', icon: Terminal },
    { id: 'DOCUMENTATION', label: 'التوثيق والدليل', icon: Book },
  ];
            ...
            <div className="relative z-10 flex items-center gap-4">
                ...
                <div>
                   ...
                   <div className="flex items-center gap-2 mt-1.5">
                       <span className={`w-1.5 h-1.5 rounded-full ${isOnline ? 'bg-success-500' : 'bg-red-500'} animate-blink`}></span>
                       <span className={`text-[10px] font-mono tracking-[0.2em] ${isOnline ? 'text-primary-500/80' : 'text-red-500'}`}>
                           {isOnline ? 'V6.0 PRODUCTION' : 'فقد الاتصال'}
                       </span>
                   </div>
                </div>
            </div>
        ...
        <div className="p-6 border-t border-white/5 bg-[#0a0f1e]/50 relative">
            <div className="flex items-center gap-4 mb-4">
                <div className="w-10 h-10 rounded-full bg-gradient-to-tr from-slate-700 to-slate-900 p-[1px] ring-2 ring-primary-500/20 relative">
                     ...
                     <div className={`absolute bottom-0 left-0 w-3 h-3 rounded-full border-2 border-slate-900 ${isOnline ? 'bg-success-500' : 'bg-red-500'}`}></div>
                </div>
      ...
      <main className="lg:mr-80 w-full flex-1 flex flex-col h-screen overflow-hidden relative z-10">
        ...
        <header className="h-16 border-b border-white/5 bg-[#020617]/90 backdrop-blur-md flex items-center justify-between px-4 lg:px-8 sticky top-0 z-20 shadow-lg">
           <div className="flex items-center gap-4 lg:gap-6">
              ...
              <div className={`flex items-center gap-3 px-4 py-1.5 rounded-full border shadow-inner transition-colors duration-500 ${isOnline ? 'bg-slate-900/80 border-white/5' : 'bg-red-500/10 border-red-500/20'}`}>
                {isOnline ? (
                    <>
                    ...
                    <>
                        <WifiOff className="w-3 h-3 text-red-500 animate-bounce" />
                        <span className="text-[10px] font-mono text-slate-400 font-bold tracking-wider hidden sm:inline">
                            الاتصال: <span className="text-red-500">غير متصل</span>
                        </span>
                    </>
                )}
              </div>
              
              {/* Production Secure Badge */}
              {isSecure ? (
                   <div className="hidden sm:flex items-center gap-2 px-3 py-1.5 rounded-full border border-green-500/10 bg-green-500/5">
                        <Lock className="w-3 h-3 text-green-400" />
                        <span className="text-[10px] text-green-400 font-mono font-bold">SECURE TLS</span>
                   </div>
              ) : (
                   <div className="hidden sm:flex items-center gap-2 px-3 py-1.5 rounded-full border border-orange-500/10 bg-orange-500/5 animate-pulse">
                        <Lock className="w-3 h-3 text-orange-400" />
                        <span className="text-[10px] text-orange-400 font-mono font-bold">UNSECURE</span>
                   </div>
              )}
              ...
              <div className="text-[10px] font-mono text-slate-500 tracking-wider hidden sm:block">
                زمن الاستجابة: <span className={isOnline ? "text-primary-400" : "text-red-500"}>{isOnline ? '24ms' : '---'}</span>
              </div>
           </div>
           ...
           <div className="font-mono text-xs text-slate-500 flex gap-4">
                ...
                <span className="text-slate-300 font-sans">{new Date().toLocaleDateString('en-US')}</span>
           </div>
        </header>
