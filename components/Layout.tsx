...
export const Layout: React.FC<LayoutProps> = ({ children, currentView, onNavigate, onReset, userProfile }) => {
  ...
  return (
    <div className="d-flex flex-column h-100">
      ...
      <nav className="navbar navbar-dark fixed-top" style={{background: 'rgba(2,4,10,0.9)', backdropFilter: 'blur(10px)', borderBottom: '1px solid rgba(255,255,255,0.05)', zIndex: 1050}}>
        <div className="container-fluid">
            <div className="d-flex align-items-center">
                ...
                <a className="navbar-brand fw-bold d-flex align-items-center" href="#">
                   ...
                   <span className="badge bg-dark border border-secondary ms-2 text-muted" style={{fontSize: '0.6rem'}}>PROD V6.2</span>
                </a>
            </div>
      ...
      <div className="container-fluid pt-5 mt-4 h-100">
          <div className="row h-100">
              ...
              <div className={`col-lg-2 col-md-3 bg-dark border-end border-secondary position-fixed h-100 d-flex flex-column p-0 start-0 transition-all ${mobileMenuOpen ? 'd-block' : 'd-none d-md-block'}`} 
                   style={{zIndex: 1060, paddingTop: '1rem', background: 'rgba(5, 8, 15, 0.98)', boxShadow: mobileMenuOpen ? '10px 0 30px rgba(0,0,0,0.5)' : 'none'}}>
                  
                  {/* Mobile Close Button */}
                  <div className="d-md-none position-absolute top-0 end-0 p-3">
                      <button className="btn btn-sm btn-outline-secondary border-0" onClick={() => setMobileMenuOpen(false)}>
                          <X size={24} />
                      </button>
                  </div>
