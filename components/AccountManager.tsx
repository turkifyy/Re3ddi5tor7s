...
import { Shield, RefreshCw, Trash2, Plus, Loader2, Brain, X, MessageSquare, Activity, Wifi, BarChart3 } from 'lucide-react';
import { isFirebaseConfigured } from '../services/firebase';
import { credentialManager } from '../services/credentialManager'; // Direct access for local pool stats

export const AccountManager: React.FC = () => {
  const [accounts, setAccounts] = useState<RedditAccount[]>([]);
  const [localPool, setLocalPool] = useState<any[]>([]); // To show daily usage from local credentials
  ...
  const fetchAccounts = async () => {
    setIsLoading(true);
    try {
      // 1. Critical: Sync usage stats from Server Bot before rendering
      await credentialManager.pullFromCloudVault();

      const data = await DatabaseService.getAccounts();
      const pool = credentialManager.getPool(); // Get updated credentials
      setLocalPool(pool);
  ...
  const handleDelete = async (id: string, username: string) => {
    if(!confirm('هل أنت متأكد من إنهاء هذه الهوية؟ سيؤدي هذا أيضاً لحذف المفاتيح المرتبطة.')) return;
    try {
      await DatabaseService.deleteAccount(id);
      
      // FIX: Clean up credentials to prevent Zombie Keys
      credentialManager.removeCredentialByUsername(username);
      
      addToast('info', 'تم إنهاء الهوية وإزالتها من الشبكة.');
      setAccounts(prev => prev.filter(a => a.id !== id));
      setLocalPool([...credentialManager.getPool()]); // Update local view
    } catch (err) {
  ...
  // Helper to match local usage data with cloud account data
      ...
      <div className="flex justify-between items-center">
        <div>
          ...
          <p className="text-slate-400 text-sm mt-2 ml-1">
            {accounts.length > 0 
              ? `إدارة ${accounts.length} هوية نشطة. الخوارزمية تعمل في وضع: Sequential Limits (100).`
          ...
          <table className="w-full text-right text-sm text-slate-400">
            <thead className="bg-white/5 text-slate-300 uppercase font-mono text-xs tracking-wider border-b border-white/10">
              <tr>
                ...
                <th className="px-8 py-5 font-semibold">الحصة اليومية (Daily Quota)</th>
            ...
            <tbody className="divide-y divide-white/5">
              {accounts.map((acc) => {
                  ...
                  const usagePercent = (usage.daily / usage.max) * 100;
                ...
                <tr key={acc.id} className="hover:bg-white/5 transition-colors group">
                  ...
                  <td className="px-8 py-5">
                    {/* NEW: Daily Usage Visualization */}
                    <div className="flex flex-col gap-1 w-32">
                        <div className="flex justify-between text-[10px] font-mono font-bold">
                            <span className={usagePercent >= 100 ? 'text-red-400' : 'text-slate-400'}>
                                {usage.daily} / {usage.max}
                            </span>
                            <span className="text-slate-600">
                                {usagePercent >= 100 ? 'MAX' : 'ACTIVE'}
                            </span>
                        </div>
                        <div className="w-full h-1.5 bg-slate-800 rounded-full overflow-hidden">
                            <div 
                                className={`h-full rounded-full transition-all duration-500 ${
                                    ...
                                    usage.daily > 0 ? 'bg-green-500' : 'bg-slate-600'
                                }`}
                  ...
                  <td className="px-8 py-5 text-slate-300 font-mono">
                    {acc.karma.toLocaleString('en-US')}
                  </td>
                  <td className="px-8 py-5">
                    {acc.sentiment ? (
