...
export const AccountManager: React.FC = () => {
  ...
  const handleAddAccount = async () => {
    ...
    // SANITIZATION: Remove leading slashes, 'u/', etc. to get raw name
    const cleanName = newUsername.replace(/^(u\/|\/u\/|\/)+/i, '').trim();
    if (!cleanName) return;

    try {
      setIsLoading(true);
      await DatabaseService.addAccount({
        username: `u/${cleanName}`,
               ...
               <div className="card-body">
                   <div className="row g-3 align-items-end">
                       <div className="col-md-5">
                           ...
                           <input type="text" className="form-control" value={newUsername} onChange={e => setNewUsername(e.target.value)} placeholder="e.g. u/username" />
                       </div>
                       <div className="col-md-5">
                           <label className="form-label text-muted">Proxy IP</label>
                           <input type="text" className="form-control" value={newProxy} onChange={e => setNewProxy(e.target.value)} placeholder="Optional" />
                       </div>
