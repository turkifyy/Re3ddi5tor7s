...
// FIX: Define explicit types for props to prevent "implicit any" indexing errors
...
const WidgetCard = ({ title, value, sub, icon: Icon, color = "primary", onClick }: WidgetCardProps) => {
    // FIX: Explicitly type the dictionary to allow string indexing
    ...
    return (
        ...
        >
            <div className={`absolute inset-0 bg-gradient-to-br ${activeColorClass.split(" ")[1]} ${activeColorClass.split(" ")[2]} opacity-0 group-hover:opacity-100 transition-opacity duration-500`}></div>
            <div className="flex justify-between items-start relative z-10">
                ...
                <div className={`p-3 rounded-xl bg-white/5 border border-white/5 ${activeColorClass.split(" ")[0]} group-hover:scale-110 transition-transform duration-300 shadow-[0_0_15px_rgba(0,0,0,0.5)]`}>
                    <Icon className="w-6 h-6" />
                </div>
            </div>
            <div className="mt-6 flex items-end justify-between relative z-10">
                <div className={`text-xs font-mono flex items-center gap-1.5 ${activeColorClass.split(" ")[0]}`}>
