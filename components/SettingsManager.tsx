...
import { Settings, Shield, Server, Wifi, Database, Activity, AlertTriangle, RefreshCw, Eye, Key, Save, Lock, MessageCircle, User, Plus, Trash2, Cpu, CheckCircle2, Zap, Clock, PlayCircle, GitBranch, Terminal, Link, UploadCloud, FileText, Check, Play } from 'lucide-react';
...
export const SettingsManager: React.FC<SettingsManagerProps> = ({ onLogout }) => {
  ...
  // UPDATED: Renamed to Dry Run Verification to indicate real code execution
  const handleDryRunVerification = () => {
      // PROD NOTE: This calls the ACTUAL 'getOptimalCredential' method used by the scraper.
      // It executes the real selection algorithm but stops short of making the network request.
      // This allows admins to verify the logic is prioritizing the correct accounts.
      
      const confirmTest = confirm("تأكيد: سيقوم النظام بتشغيل خوارزمية الاختيار (Selection Algo) فعلياً وتحديث العداد المحلي. هذا ليس اختباراً وهمياً.");
      if (!confirmTest) return;

      const selected = credentialManager.getOptimalCredential();
      
      if (selected) {
          const msg = `Algorithm Result: Selected Node [${selected.username}] (Usage: ${selected.dailyUsage}/100)`;
          logger.info('SYS', msg);
          addToast('success', `نجح التحقق: الخوارزمية اختارت ${selected.username} بناءً على الأولويات.`);
      } else {
          const msg = "Algorithm Result: No Nodes Available (All capped or rate-limited).";
          logger.error('SYS', msg);
          addToast('error', 'فشل التحقق: الخوارزمية لم تجد أي حساب متاح.');
      }
      loadCredentials(); // Refresh UI to show simulated increment
  };
      ...
      <div className="grid grid-cols-1 lg:grid-cols-2 gap-8">
        ...
        <div className="glass-panel rounded-2xl p-8 border border-white/5 lg:col-span-2">
            <div className="flex justify-between items-center mb-6">
                ...
                <div className="flex gap-2">
                    <Button size="sm" onClick={handleDryRunVerification} variant="secondary" className="border-orange-500/30 text-orange-400 hover:text-orange-300">
                         <Play className="w-3 h-3 ml-2" />
                         فحص الخوارزمية (Algo Dry-Run)
                    </Button>
                             ...
                             <div className="flex items-center gap-4 w-full md:w-auto mb-3 md:mb-0">
                                ...
                                <div>
                                    ...
                                    <div className="flex items-center gap-3 text-[10px] text-slate-500 mt-1 font-mono">
                                        ...
                                        <span className={cred.dailyUsage >= 100 ? 'text-red-400' : 'text-slate-400'}>Usage: {cred.dailyUsage}/100</span>
                                    </div>
                                </div>
                             </div>
