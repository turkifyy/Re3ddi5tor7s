...
export const SettingsManager: React.FC<SettingsManagerProps> = ({ onLogout }) => {
  ...
  // NEW: Robust Simulation Logic for Verification
  const handleSimulateRotation = () => {
      // We do NOT modify state in simulation, we just query
      // To simulate, we'll temporarily clone the pool but CredentialManager logic is tied to instance
      // So we just call getOptimalCredential but DO NOT execute any API call
      // NOTE: getOptimalCredential WILL increment the counter in memory, so we must warn user.
      
      const confirmTest = confirm("تحذير: المحاكاة ستقوم بزيادة العداد المحلي لحساب واحد لاختبار الخوارزمية. هل أنت متأكد؟");
      if (!confirmTest) return;

      const selected = credentialManager.getOptimalCredential();
      
      if (selected) {
          const msg = `Simulation Result: Selected [${selected.username}] (Usage: ${selected.dailyUsage}/100)`;
          logger.info('SYS', msg);
          addToast('success', `نجح الاختبار: تم اختيار ${selected.username} بناءً على التسلسل.`);
      } else {
          const msg = "Simulation Failed: System Halted (All accounts capped or rate-limited).";
          logger.error('SYS', msg);
          addToast('error', 'فشل الاختبار: لا توجد حسابات متاحة.');
      }
      ...
      <div className="grid grid-cols-1 lg:grid-cols-2 gap-8">
        ...
        <div className="glass-panel rounded-2xl p-8 border border-white/5 lg:col-span-2">
            <div className="flex justify-between items-center mb-6">
                ...
                <div className="flex gap-2">
                    <Button size="sm" onClick={handleSimulateRotation} variant="secondary" className="border-orange-500/30 text-orange-400 hover:text-orange-300">
                         <Play className="w-3 h-3 ml-2" />
                         فحص الخوارزمية (Simulation)
                    </Button>
