...
import { Settings, Server, Key, Database, UploadCloud, Trash2, Youtube, RefreshCw, AlertTriangle } from 'lucide-react';
...
export const SettingsManager: React.FC<SettingsProps> = ({ onLogout }) => {
  ...
  const handleSaveFb = async () => {
      // AUTO-TRIM INPUTS
      const cleanPid = fbProjectId.trim();
      const cleanKey = fbApiKey.trim();

      if (!cleanPid || !cleanKey) {
          addToast('error', 'Project ID and API Key required');
          return;
      }

      const success = await initializeFirebase({ apiKey: cleanKey, projectId: cleanPid, authDomain: `${cleanPid}.firebaseapp.com` });
      if (success) {
  ...
  const handleSaveDs = () => {
      const cleanKey = dsKey.trim();
      if(!cleanKey.startsWith('sk-')) {
          addToast('error', 'Invalid Key Format (must start with sk-)');
          return;
      }
      setDeepSeekKey(cleanKey);
      setDsKey(cleanKey); // Update UI
      addToast('success', 'DeepSeek Key Saved');
  };

  const handleSaveYt = () => {
      const cleanKey = ytKey.trim();
      // VALIDATION FIX: Google API keys typically start with AIza
  ...
  // MAINTENANCE: Emergency Cache Flush
  const handleSystemFlush = () => {
      if(confirm('WARNING: This will clear all local settings, keys, and cached tokens. The system will reset to factory state. Continue?')) {
          localStorage.clear();
          window.location.reload();
      }
  };
  ...
  return (
    <div className="container-fluid p-0">
        <div className="d-flex justify-content-between align-items-center mb-4">
            <h3 className="fw-bold text-white mb-0"><Settings className="me-2"/> Settings & Configuration</h3>
            <button className="btn btn-outline-danger btn-sm d-flex align-items-center" onClick={handleSystemFlush} title="Emergency Reset">
                <RefreshCw size={14} className="me-2"/> System Flush
            </button>
        </div>
        ...
        <div className="row mb-4">
            ...
            {/* YouTube Info */}
             <div className="col-lg-4 mb-4">
                <div className="card h-100">
                    <div className="card-body">
                        <h5 className="card-title fw-bold mb-3"><Youtube size={20} className="me-2 text-danger"/> YouTube API</h5>
                        <div className="mb-3">
                            <label className="form-label text-muted">Data API V3 Key</label>
                            <input type="password" className="form-control" value={ytKey} onChange={e => setYtKey(e.target.value)} placeholder="AIza..." />
                        </div>
                        <Button onClick={handleSaveYt}>Save Key</Button>
                        <div className="text-muted small mt-3">Required for Video Comment Scraping.</div>
                    </div>
                </div>
            </div>
        </div>
