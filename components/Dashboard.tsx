...
export const Dashboard: React.FC<DashboardProps> = ({ onNavigate }) => {
  ...
  const isFetching = useRef(false); // MUTEX LOCK

  const fetchData = useCallback(async () => {
    // If a request is already in flight, skip this tick to prevent pile-up
    ...
    try {
        ...
        if (isMounted.current) {
            setStats({ 
                ...
                leads: leadsCount, 
                engagement: globalStats.totalEngagements, 
                aiOps: globalStats.aiOps 
            });
  ...
  useEffect(() => {
    ...
    const interval = setInterval(() => {
      ...
      setLiveData(prev => {
        ...
        // Keep array size fixed to prevent memory leaks in long-running sessions
        const newArr = [...prev.slice(1), { time: nextTime, value: realOps }];
        return newArr;
      });
      
      fetchData(); // Protected by isFetching mutex
    }, 2000);
  ...
  return (
    <div className="container-fluid p-0">
        ...
        <div className="row">
            ...
            <WidgetCard title="Engagement" value={stats.engagement} sub="Replies Deployed" icon={Target} colorClass="text-info" onClick={() => onNavigate?.('INBOX')} />
        </div>
                ...
                <div className="card mb-4" style={{height: '400px'}}>
                    <div className="card-body">
                        ...
                        <div style={{ height: '300px' }}>
                            {/* CONSOLE FIX: Only render chart if data exists to prevent Recharts warnings */}
                            {liveData && liveData.length > 0 ? (
                                <ResponsiveContainer width="100%" height="100%">
                                    <AreaChart data={liveData}>
                                        ...
                                        <Area type="monotone" dataKey="value" stroke="#00e5ff" strokeWidth={3} fillOpacity={1} fill="url(#colorValue)" isAnimationActive={false} />
                                    </AreaChart>
                                </ResponsiveContainer>
                            ) : (
