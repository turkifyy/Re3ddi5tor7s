  ...
  useEffect(() => {
    ...
    const interval = setInterval(() => {
      ...
      setLiveData(prev => {
        ...
        let chartValue = realOps > 0 ? Math.min(100, realOps * 25) : Math.max(0, prev[prev.length-1].value - 10);
  ...
  return (
    <div className="space-y-8 pb-20">
      ...
      <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-6">
        <WidgetCard title="الفرص المكتشفة" value={stats.leads} sub="نتائج الزحف (Scraped)" icon={Search} color="success" delay={0} onClick={() => onNavigate?.('SCRAPER')} />
        <WidgetCard title="الهويات النشطة" value={stats.accounts} sub="عمليات الشبكة" icon={Users} color="violet" delay={100} onClick={() => onNavigate?.('ACCOUNTS')} />
        <WidgetCard title="عمليات DEEPSEEK" value={stats.aiOps} sub="توكنز" icon={Zap} color="warning" delay={200} onClick={() => onNavigate?.('CAMPAIGNS')} />
        <WidgetCard title="إجمالي التفاعل" value={stats.engagement} sub="ردود منشورة" icon={Target} color="primary" delay={300} onClick={() => onNavigate?.('CAMPAIGNS')} />
      </div>
        ...
        <div className="lg:col-span-3 glass-panel rounded-2xl p-8 border border-white/5 relative overflow-hidden">
             ...
             <div className="grid grid-cols-1 md:grid-cols-4 gap-6 relative z-10">
                ...
                <div className="p-4 bg-white/5 rounded-xl border border-white/5">
                    ...
                    <div className="flex items-end gap-2"><span className={`text-2xl font-mono font-black ${redditHealth.averageLatency > 1000 ? 'text-orange-400' : 'text-white'}`}>{Math.round(redditHealth.averageLatency)}</span><span className="text-[10px] text-slate-500 mb-1">ms</span></div>
      ...
      <div className="glass-panel rounded-2xl p-6 border border-primary-500/10 relative overflow-hidden">
           ...
           <div className="flex justify-between items-center mb-6 relative z-10">
               ...
               <div className="text-[10px] text-slate-500 font-mono bg-white/5 px-2 py-1 rounded border border-white/5 flex items-center gap-2"><Clock className="w-3 h-3" /> الصيانة القادمة: {roboState?.nextMaintenance}</div>
           </div>
               ...
               {agents.map((agent) => (
                   <div key={agent.id} className="bg-[#0b0f19] border border-white/5 rounded-xl p-4 hover:border-primary-500/30 transition-all group">
                       <div className="flex justify-between items-start mb-3">
                           ...
                           <div className={`w-1.5 h-1.5 rounded-full ${agent.status === 'WORKING' ? 'bg-success-500 animate-pulse' : agent.status === 'RESTING' ? 'bg-orange-500' : agent.status === 'ERROR' ? 'bg-red-500' : 'bg-slate-700'}`}></div>
                       </div>
      ...
      <div className="grid grid-cols-1 lg:grid-cols-3 gap-8">
        <div className="lg:col-span-2 glass-panel rounded-2xl p-1 relative overflow-hidden group">
          ...
          <div className="p-6 border-b border-white/5 flex justify-between items-center bg-[#0f172a]/50">
            <div className="flex items-center gap-3"><div className="p-2 bg-primary-500/10 rounded-lg"><Wifi className="w-5 h-5 text-primary-400" /></div><div><h3 className="text-sm font-bold text-white">نشاط النظام (System Heartbeat)</h3><div className="text-[10px] text-primary-400 font-mono">عملية/ثانية</div></div></div>
          ...
          <div className="h-[350px] w-full p-4 bg-[#020617]/40 relative">
            <ResponsiveContainer width="100%" height="100%">
              <AreaChart data={liveData}>
                ...
                <Area type="monotone" dataKey="value" stroke="#00f0ff" strokeWidth={3} fillOpacity={1} fill="url(#colorValue)" animationDuration={300} isAnimationActive={true} />
              </AreaChart>
            </ResponsiveContainer>
