...
import { RedditAccount, Campaign, AccountStatus, ServerPulse, ScrapedLead } from '../types';
...
export const DatabaseService = {
  ...
  // NEW: Get Deployment History (Moved from CampaignManager)
  ...
  // Optimized: Client-side sorting with safety check for dates
  async getPendingLeads(): Promise<ScrapedLead[]> {
    return withTracking(async () => {
        const db = ensureConnection();
        if (!db) return [];
        
        // Fetch leads marked as NEW
