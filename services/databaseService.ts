...
const handleError = (e: any, context: string) => {
    ...
    if (e.code === 'failed-precondition') msg = "الاستعلام يتطلب فهرساً (Index). جاري التبديل للوضع الآمن.";
    
    // Prevent logging error loops if the logger itself is causing the error
    if (context !== 'WriteSystemLog') {
...
export const DatabaseService = {
  ...
  async deployCampaignContent(campaignId: string | undefined, content: string, subreddit: string): Promise<void> {
    return withTracking(async () => {
        ...
        const statsRef = doc(db, METRICS_COL, 'global_stats');
        await setDoc(statsRef, { totalEngagements: increment(1) }, { merge: true });

        logger.success('DB', `تم نشر المحتوى وتحديث العداد المركزي.`);
    }, 'DeployContent');
  },
    ...
    return withTracking(async () => {
        ...
        try {
            ...
            return snapshot.docs.map(doc => ({ id: doc.id, ...doc.data() }));
             ...
             // FALLBACK: If index missing, sort client side SAFELY
             if (e.code === 'failed-precondition') {
                 logger.warn('DB', 'History Index missing. Using Safe Mode.');
                 const q = query(collection(db, CONTENT_COL), limit(limitCount));
                 const snapshot = await getDocs(q);
                 return snapshot.docs
                    .map(doc => ({ id: doc.id, ...doc.data() }))
                    // SAFE SORT: Handle null/pending timestamps preventing crash
                    .sort((a: any, b: any) => {
                        const tA = a.deployedAt?.seconds || 0;
                        const tB = b.deployedAt?.seconds || 0;
                        return tB - tA;
                    });
             }
             throw e;
  ...
  async getPendingLeads(): Promise<ScrapedLead[]> {
    return withTracking(async () => {
        ...
        try {
             ...
             const snap = await getDocs(q);
             return snap.docs.map(doc => ({id: doc.id, ...doc.data()} as ScrapedLead));
        } catch(e: any) {
             if (e.code === 'failed-precondition') {
                 logger.warn('DB', 'Leads Index missing. Using Safe Mode.');
                 const q = query(collection(db, LEADS_COL), where('status', '==', 'NEW'), limit(100));
                 const snap = await getDocs(q);
                 const leads = snap.docs.map(doc => ({id: doc.id, ...doc.data()} as ScrapedLead));
  ...
  async markLeadDismissed(id: string): Promise<void> {
    return withTracking(async () => {
        const db = ensureConnection();
        if (!db) return;
        ...
        if (!snap.empty) {
            ...
            logger.info('DB', `Lead ${id} marked as Dismissed.`);
        }
    }, 'MarkLeadDismissed');
  },
  ...
  // --- Metrics ---
  async getGlobalStats(): Promise<{ aiOps: number, totalEngagements: number }> {
    return withTracking(async () => {
        const db = ensureConnection();
        if (!db) return { aiOps: 0, totalEngagements: 0 };
        ...
        return { aiOps: 0, totalEngagements: 0 };
    }, 'GetGlobalStats', { aiOps: 0, totalEngagements: 0 });
  },
