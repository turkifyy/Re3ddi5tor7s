...
export const AnalyticsEngine = {
  ...
  predictVirality(content: string): { score: number; rating: string; color: string } {
      if (!content) return { score: 0, rating: 'N/A', color: 'text-muted' };
      ...
      let rating = "Standard";
      let color = "text-muted"; // Default Bootstrap class

      // Updated to use Bootstrap Classes
      if (finalScore >= 80) { rating = "High Impact"; color = "text-purple"; } // Custom class in index.html
      else if (finalScore >= 60) { rating = "Optimized"; color = "text-success"; }
      else if (finalScore >= 40) { rating = "Acceptable"; color = "text-secondary"; }
      else { rating = "Low Quality"; color = "text-warning"; }
