...
export const AnalyticsEngine = {
  ...
  calculateAccountHealth(account: RedditAccount): number {
    ...
    // Parse ISO date
    const lastActiveDate = new Date(account.lastActive);
    const now = new Date();
    
    // If invalid date (e.g. "الآن" or initial string), treat as recent if it matches, else check time
    if (account.lastActive === 'الآن') {
        score += 5;
    } else if (!isNaN(lastActiveDate.getTime())) {
        const hoursDiff = (now.getTime() - lastActiveDate.getTime()) / (1000 * 60 * 60);
        if (hoursDiff < 24) score += 5; // Active in last 24h
        else if (hoursDiff > 168) score -= 10; // Inactive for a week
    }
