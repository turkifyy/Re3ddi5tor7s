...
export interface RedditCredential {
    ...
    usageCount: number; // Total lifetime usage
    dailyUsage: number; // NEW: Usage count for today
    dayStartTimestamp: number; // NEW: When the current "day" started
    status: 'READY' | 'RATE_LIMITED' | 'EXHAUSTED' | 'DAILY_CAP_REACHED'; // Updated status
    lastUsed: number;
...
export interface ScrapedLead {
    id: string; // Reddit Thing ID (t1_xyz or t3_xyz) or YouTube Comment ID
    type: 'POST' | 'COMMENT';
    subreddit: string; // Or "YouTube: Video Title"
    author: string;
    content: string;
...
export interface SystemLog {
  ...
  module: 'AAO' | 'CASE' | 'SYS' | 'NET' | 'DB' | 'AI' | 'AUTH' | 'REDDIT' | 'BOT' | 'CRON' | 'SCRAPER' | 'SEC' | 'YOUTUBE';
  message: string;
}
...
// NEW: Marketing Types
export type MarketingCategory = 'MOVIES' | 'SERIES' | 'MATCHES' | 'RECIPES' | 'GAMES' | 'APPS' | 'CUSTOM' | 'YOUTUBE';
// Updated Timeframes to support requirements
export type SearchTimeframe = '24h' | 'week' | 'month' | '3months' | 'year' | 'all' | 'custom';
